...
export class ProductsGrid extends ComponentV2 {
  ...
  #allProducts = []; // Store the fetched products
...
async render() {
  try {
    ...
    const searchText = searchParams.get('search') || ''; // Get initial search term
    ...
    this.#allProducts = productsData;  // Store all fetched products in memory
    ...
    this.#renderProducts(productsData);  // Render products grid
...
// Handle the search input and filter products
#handleSearchInput(event) {
  const searchText = event.target.value.toLowerCase();  // Get the input value
  const filteredProducts = this.#allProducts.filter((product) => {
    return product.name.toLowerCase().includes(searchText) || 
           (product.keywords && product.keywords.some(keyword => keyword.toLowerCase().includes(searchText)));
  });

  this.#renderProducts(filteredProducts);  // Render the filtered products
}
